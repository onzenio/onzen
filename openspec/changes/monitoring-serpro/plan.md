...
- `app/Services/AuditService.php` mínimo: `record(int $action account..., ?User $actor, string $action, array $metadata = [])` gravando `AuditLog` (origin_account_id obrigatório; para eventos de plataforma usar a Account A) sem lançar exceção em falha (best-effort, log técnico). Este é o bridge até o `AuditObserver` do multi-account.
