...
- `app/Services/AuditService.php` mínimo: `record(?User $actor, string $action, array $metadata = [], ?Account $account = null): ?AuditLog` gravando `AuditLog` (origin_account_id = a Account afetada: `$account` > account do ator > CurrentAccount > Account A; retorno null sinaliza falha best-effort) sem lançar exceção em falha (best-effort, log técnico; insert em savepoint para não abortar transação externa em Postgres). Callers devem passar a Account afetada. Este é o bridge até o `AuditObserver` do multi-account.
