...
- Módulo Go `onefisc/wzap`; Go 1.26; **`CGO_ENABLED=0`**; sem SQLite, sem Redis.
...
```bash
cd wzap && go mod init onefisc/wzap && go build ./...
```
...
- Compose: serviço `wzap` (`build: ./wzap`, `127.0.0.1:8081:8080`, env `WZAP_SERVICE_TOKEN`, `WZAP_DATABASE_URL=postgres://onefisc:secret@postgres:5432/onefisc_wzap?sslmode=disable`, `WZAP_NATS_URL=nats://nats:4222`, `WZAP_AUTO_MIGRATE=true`, `WZAP_DATA_DIR=/data`, volume `wzap-media:/data`, `depends_on` postgres/nats `service_healthy`, `restart: unless-stopped`).
- `postgres-init.sql`: `CREATE DATABASE onefisc_wzap;` montado em `docker-entrypoint-initdb.d` do serviço postgres (vale para volumes novos).
