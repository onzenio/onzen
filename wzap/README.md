...
Gateway interno de WhatsApp do OneFisc. O wzap mantém sessões multi-instância
do WhatsApp, expõe um contrato REST de comandos para o `backend/` e publica os
eventos recebidos (mensagens, recibos, conexão e status de envio) em um stream
durável do NATS JetStream.

## Visão e escopo

- Serviço Go interno (`module onefisc/wzap`), consumido **somente** pelo
  `backend/` Laravel com um token único de serviço. Não conhece usuários,
  roles, Account ou Client: a instância é identificada por `id` e por um
  `external_ref` opcional e único cujo significado vive no Laravel.
...
SQLite, Redis, métricas Prometheus, escala horizontal e o consumidor Laravel
(change futura). O serviço roda como **uma réplica**; locks são em memória.

## Requisitos

- Go 1.26+ (o `go.mod` pina `go 1.26.0`) para build/testes locais.
- Postgres 18 (banco `onefisc_wzap`; outro banco `_test` para os testes de
...
| `WZAP_DATABASE_URL` | sim | — | URL do Postgres, ex. `postgres://onefisc:secret@postgres:5432/onefisc_wzap?sslmode=disable`. |
...
O serviço está no `docker-compose.yml` da raiz como `wzap`, publicado em
`127.0.0.1:8081`, com o volume `wzap-media` e dependências Postgres/NATS.
...
`WZAP_DATABASE_URL` apontando para o banco `onefisc_wzap` dentro da rede,
...
eventos apontar para o endereço que o Laravel alcança; ajuste-o no compose (ou
com um override) se o backend não enxergar `127.0.0.1:8081`.

**Banco em volume pré-existente:** o script `wzap/docker/postgres-init.sql`
cria `onefisc_wzap` automaticamente apenas quando o volume do Postgres é
inicializado do zero. Em um volume que já existe (como o do ambiente de dev),
crie o banco uma única vez:

```bash
docker compose exec postgres createdb -U onefisc onefisc_wzap
```
...
```bash
docker compose exec postgres createdb -U onefisc onefisc_wzap_test
```

```bash
cd wzap

# suíte completa (o go fica em /usr/local/go fora do PATH do compose)
WZAP_TEST_DATABASE_URL='postgres://onefisc:secret@127.0.0.1:5432/onefisc_wzap_test?sslmode=disable' \
...
No CI (`.github/workflows/wzap.yml`, filtro `wzap/**`) rodam `go vet`,
...
- Licenças e trechos reaproveitados: `wzap/THIRD_PARTY_NOTICES.md`.
