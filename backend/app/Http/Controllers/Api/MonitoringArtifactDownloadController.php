...
class MonitoringArtifactDownloadController extends Controller
{
    ...
    public function __invoke(Request $request, string $ref, ArtifactStore $store, AuditService $audit): Response
    {
        ...
        try {
            ...
            $audit->record($user, (int) $artifact->account_id, null, 'monitoring.artifact.download_failed', [
                'ref' => $artifact->ref,
        ...
        $audit->record($user, (int) $artifact->account_id, null, 'monitoring.artifact.downloaded', [
            'ref' => $artifact->ref,
            'kind' => $artifact->kind,
